...
namespace Napoleon.Tests
{
    ...
    public class RequestReplyTests
    {
        ...
        public async Task Trying_to_write_data_on_a_server_which_is_not_leader_fails()
        {
            ...
            Assert.ThrowsAsync<NotSupportedException>(async() =>
            {
                await dataClient.PutValue("col1", "key1", JsonSerializer.SerializeToNode("value02")!, CancellationToken.None);
            });
        ...
        public async Task Synchronize_data_stores_with_tcp_when_a_single_key_changed()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                dataStore.PutSimpleValue("collection1", "key001",i);
            }
            ...
            {
                clientData.ApplyChanges(new []{change});
        ...
        public async Task Synchronize_data_stores_with_tcp_when_multiple_keys_changed()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                dataStore.PutSimpleValue("collection1", $"key{i}",i);
            }
